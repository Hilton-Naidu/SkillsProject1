...
This tool is incredibly important to the role of Computer Scientist, because it allows integration and development of real time functioning systems, capable of sending a receiving online data. For the development of a system, latency and interactivity matter to a Computer Scientist, and Python WebSocket allows for things such as real time data feeds and low-latency communication. This would allow for a computer scientist to experiment with how systems and algorithms might interact with live user data. It also allows for a Computer Scientist to design and test custom communication protocols over TCP, such as defining message formats and encryption layers. A Computer scientist would also importantly allow for Async Paradigms, which would allow for the handling of multiple clients to access information from a central server, and it would also provide a lightweight mechanism to scale well. This is a tool which would allow a computer scientist to powerfully develop the backend systems for a web based application, using the python programming language, allowing for the integration of systems, and the use of Async Paradigms. 


There are a few significant limitations or weaknesses of, 3 being:
- Python Websockets is a single threaded and Event-Loop Dependant. Since the library is designed around asyncio (single event loop) and does not support multithread processing, there is significant reduction in it’s capabilities to easily scale. For a high traffic server, it should be considered that using a single thread could cause a bottleneck, as many different clients are waiting to asynconously call on the server through only one thread, additional infrastructure would be required to fix this. 

- It lacks some significant high level features without extra work, such as broadcasting to multiple clients, session management, rooms, and automatic reconnection logic for clients. All these functions are able to be developed through the framework, but it still it requires significantly more development time to manually develop these useful high level functions. 

- Security, while the package is capable of supporting secure websockets, it falls short in multiple other important areas, such as authentication layers (Jet, oAuth), Rate limiting, and message validation or encryption beyond TLS. To provide an adequate amount of security other external tools must be integrated and additional code must be added to ensure secure WebSocket channels. 
